...
template <typename T> using Uptr = std::unique_ptr<T, Deleter>;

using Str = Uptr<char>;

inline Str escapeIdentifier(PGconn &connection,
                            const std::string_view identifier) {
  return Str{
      PQescapeIdentifier(&connection, identifier.data(), identifier.size())};
}

using Result =
    std::unique_ptr<PGresult,
                    decltype([](PGresult *result) { PQclear(result); })>;
...
using Connection =
    std::unique_ptr<PGconn, decltype([](PGconn *conn) { PQfinish(conn); })>;

inline Connection connect(const std::string &connectionStr) {
  Connection conn{PQconnectdb(connectionStr.c_str())};
  if (PQstatus(conn.get()) != CONNECTION_OK) {
    throw std::runtime_error{
        fmt::format("Failed to connect to db: {}", PQerrorMessage(conn.get()))};
  }
  return conn;
}
