...
make_core_name_skip(std::size_t size_at_begin, std::size_t size_at_end,
...
#define BOOST_PFR_FUNCTION_SIGNATURE ""
  ...
  static_assert(!sv.empty(), "====================> Boost.PFR: Field "
                             "reflection parser configured in a wrong way. "
                             "Please define the BOOST_PFR_FUNCTION_SIGNATURE "
                             "to a compiler specific macro, "
                             "that outputs the whole function signature "
                             "including non-type template parameters.");

  constexpr auto skip = detail::make_core_name_skip BOOST_PFR_CORE_NAME_PARSING;
  static_assert(skip.size_at_begin + skip.size_at_end +
                        skip.until_runtime.size() <
                    sv.size(),
...
template <class T> struct clang_wrapper_t {
  T v;
};
template <class T> clang_wrapper_t(T) -> clang_wrapper_t<T>;

template <class T> constexpr auto make_clang_wrapper(const T &arg) noexcept {
...
template <class T> constexpr auto tie_as_names_tuple() noexcept {
  static_assert(!std::is_union<T>::value,
                "====================> Boost.PFR: For safety reasons it is "
                "forbidden to reflect unions. See `Reflection of unions` "
                "section in the docs for more info.");
