...
void test_get_name_by_type() {
  ...
  //     BOOST_TEST_EQ( ((boost::pfr::get_name<nonconstexpr, Aggregate>())),
  //     "this_is_a_name"); BOOST_TEST_EQ( ((boost::pfr::get_name<char_ref,
  //     Aggregate>())), "c");
}

void test_get_name_by_type_without_linkage() {
  // FIXME: implement this
  //     BOOST_TEST_EQ( ((boost::pfr::get_name<int, unnamed_t>())),
  //     "unnamed_first"); BOOST_TEST_EQ( ((boost::pfr::get_name<float,
  //     unnamed_t>())), "unnamed_second"); BOOST_TEST_EQ(
  //     ((boost::pfr::get_name<int, inside_unnamed_ns>())), "hidden");
}

void test_names_as_array() {
  const auto expected = std::array<std::string_view, 4>{
      "member1", "this_is_a_name", "c", "Forth"};
...
#ifndef _MSC_VER
  const auto expected =
      std::array<std::string_view, 2>{"unnamed_first", "unnamed_second"};
