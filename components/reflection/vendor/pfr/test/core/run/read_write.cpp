...
template <class T> void test_write_read(const T &value) {
  T result;
  std::stringstream ss;
...
template <class T> void to_string_test(const T &value, const char *ethalon) {
  std::stringstream ss;
  ss << boost::pfr::io(value);
  BOOST_TEST_EQ(ss.str(), ethalon);
}

template <class T> void test_type(const T &value, const char *ethalon) {
...
int main() {
  ...
  test_type(test2{}, "{{with_operator}, {with_operator}, {with_operator}, "
