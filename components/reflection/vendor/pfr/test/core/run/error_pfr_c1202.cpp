...
template <typename T> class CfgAttrib {
public:
  using value_type = T;
...
template <class T> void printer(const T &value, std::true_type) {
  std::cout << "- " << value.getAttrName() << ": " << value.getValue()
            << std::ends;
}

template <class T> void printer(const T &value, std::false_type) {
